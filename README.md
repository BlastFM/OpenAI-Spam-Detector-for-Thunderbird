...
This extension operates strictly as a local Thunderbird WebExtension using standard permissions:messagesRead & messagesMove: Inspect incoming message headers/snippets and move flagged messages.accountsRead & accountsFolders: Resolve folder structures across configured email accounts.menus: Add training controls to message list context menus.storage: Persist local API keys, settings, custom prompt rules, and classification memory.downloads & tabs: Handle configuration export files and UI navigation.Host Permission ([https://api.openai.com/](https://api.openai.com/)*): Transmit snippet payloads to OpenAI endpoints.Privacy Notice: Transmitted data includes email sender headers, subject line, and the first 1,000 characters of the body text. Data is sent directly to OpenAI's API according to their Data Usage Policies. No intermediate third-party servers are used.📄 LicenseDistributed under the MIT License. See LICENSE for details.
